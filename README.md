...
- 모든 스테이지에 `옥상 고양이 고립`이 다시 발생합니다. BUDDY를 좌우로 움직여 1초 뒤 추락하는 고양이를 쿠션으로 받는 전용 팝업 미니게임이며, 놓치면 사고가 남아 다시 도전할 수 있습니다.
- 모든 스테이지에 `광장 폭탄 위협`도 다시 발생합니다. FIX가 도착한 뒤 본부 여성형 AI `루나`의 새 무전 암호를 듣고 빨강·파랑 전선 중 하나를 클릭하며, 정답 신호는 시도마다 달라집니다.
- 3분 30초 동안 화재, 침수, 복합 재난의 3개 웨이브가 순서대로 진행됩니다. 고양이 구조와 폭탄 해체를 포함해 현재 웨이브의 재난을 모두 해결하면 마을 뉴스와 함께 다음 스테이지로 출동합니다.
- 실제 판정에 사용하는 사고 유형은 11개이며, 반복 미니게임을 포함한 한 판의 스테이지 임무는 총 15개입니다. 연쇄 사고 체인은 2개입니다.
- 행동 순서로 판정하는 로봇 콤보 5개와 선택형 대화 이벤트 4개가 있습니다.
- 타이틀·구조 작전·스테이지 완료 뉴스페이퍼에 서로 다른 오리지널 8비트 BGM을 사용하고 선택·출동·해결·콤보·결과 효과음을 Web Audio로 실시간 합성합니다. 뉴스 화면에서는 경쾌한 마무리 테마가 재생되고 다음 스테이지 출동 시 작전 BGM으로 복귀합니다.
