...
import { collectPageErrors, fulfillBombHint, fulfillDialogue, fulfillNews, fulfillQuiz, performAction } from "./qa_helpers.mjs";
...
await page.route("**/api/news", (route) => fulfillNews(route, "openai"));
await page.goto(`${baseUrl}/?screen=play&skipBriefing=1&tickScale=4`, { waitUntil: "networkidle" });
await page.locator("canvas").waitFor();

const recurringMiniGames = [
...
const waveThreeActions = [
  ...
  recurringMiniGames[0],
  { incidentName: "동쪽 주민 고립", incidentId: "east_residents", robot: "BUDDY", actionName: "고립 주민 구조", actionId: "rescue_residents" },
  recurringMiniGames[1],
];

async function completeWave(actions, completedWave) {
  for (const action of actions) await performAction(page, action);
  if (completedWave >= 3) return;
  const news = page.locator(`[data-stage-news="${completedWave}"]`);
  await news.waitFor({ state: "visible", timeout: 5_000 });
  await page.getByRole("button", { name: new RegExp(`WAVE ${completedWave + 1}`) }).click();
  await page.locator(`.game-screen[data-wave="${completedWave + 1}"]`).waitFor();
}

await completeWave(waveOneActions, 1);
await completeWave(waveTwoActions, 2);
await completeWave(waveThreeActions, 3);
...
await browser.close();
console.log("Click full-flow PASSED: 15 stage missions → 6 mini-game rounds → 5 combos → result → retry");
