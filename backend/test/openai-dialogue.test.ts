...
test("NPC 요청은 캐릭터 특성과 현재 게임 사실을 생성형 AI 프롬프트에 전달한다", async () => {
  ...
  const result = await writer.write({
    ...
    choiceIds: [],
    dialogueSequence: 2,
    excludedDialogues: ["강물은 빠르지만 남쪽 산책로는 아직 안전해요."],
    language: "ko",
  });

  assert.equal(result.source, "openai");
  assert.match(String(captured.instructions), /주민 캐릭터 대사 작가/);
  assert.match(String(captured.instructions), /절대 다시 쓰지 마세요/);
  assert.match(String(captured.input), /characterTraits/);
  assert.match(String(captured.input), /공원 관리인/);
  assert.equal(captured.store, false);
});

test("NPC AI가 이전 대사를 반복하면 응답을 채택하지 않는다", async () => {
  const repeated = "강물은 빠르지만 남쪽 산책로는 아직 안전해요.";
  const client = { responses: { create: async () => ({ output_text: JSON.stringify({ dialogue: repeated }) }) } } as unknown as OpenAI;
  const writer = createOpenAIDialogueWriter(config, client);
  const result = await writer.write({
    speaker: "주민", personality: "observant", situation: "npc_duri", facts: {}, choiceIds: [],
    dialogueSequence: 2, excludedDialogues: [repeated], language: "ko",
  });
