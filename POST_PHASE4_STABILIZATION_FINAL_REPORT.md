...
## 4. 폐기된 구조와 현재 목표 OCI 구조

```text
브라우저 → Vercel same-origin /api/plan
        → 공유 Bearer 토큰 → pixel-panic-api-prod-01 공인 IP:8080
        → OpenAI API
```

초기 브라우저 직접 OCI 호출 설계는 폐기됐다. 현재 브라우저는 OCI 주소나 인증 토큰을 받지 않으며 Vercel Route만 `/v1/plan`을 호출한다.
...
- 상태·Vercel 프록시 단위 테스트 14개 통과, 24개 priority 순열 포함
- backend 테스트 16개 통과, 공유 토큰 인증 포함
- Next production build 통과
- full-flow, fallback, 대표 priority 4종, 종료 경합 Playwright 통과
- 깨끗한 복사본 bootstrap 7.62초, 전체 CI 55.98초
- 로컬 Vercel 프록시→backend TCP 인증과 keyless OpenAI fallback 통과

## 7. 장애 결과

| 상황 | 결과 |
|---|---|
| OpenAI 키 없음, 올바른 공유 토큰 | backend 200 fallback, 게임 LOCAL 전체 성공 |
| 공유 토큰 없음/오류 | backend 401, planner 호출 차단 |
...
- 코드: 직전 known-good 커밋으로 복귀
- Vercel: 직전 known-good deployment를 promote
- OCI: `/opt/pixel-panic`에서 직전 승인 커밋 checkout 후 systemd reload
...
- [x] 시간 초과·포기·성공 경합 차단
- [x] HUD와 결과 통계 일치
- [x] 공유 토큰·rate limit·cache·secret-safe logging 로컬 통과
- [x] 깨끗한 소스 복사본 전체 검증
- [x] 기존 12개 아트 기준 보존 및 추가 5개 검수
- [ ] OCI VM 직접 배포와 재부팅 검증
- [ ] Vercel 서버 프록시를 OCI VM에 연결하고 Vercel OpenAI 키 제거
